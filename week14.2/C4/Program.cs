...
// }
IEnumerable<Restaurant> resturant =
    from plaats in restaurants
    where plaats.City == "Gouda" || plaats.City == "Utrecht"
    orderby plaats.OpeningYear
    select plaats;
List<Restaurant> res = resturant.ToList();
foreach (Restaurant restaurant in res)
{
    string gegevens = restaurant.ToString();
